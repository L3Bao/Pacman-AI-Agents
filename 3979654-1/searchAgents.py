...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space) => state space is a tuple consisting of ((x,y),(corners visited))
        """
        "*** YOUR CODE HERE ***"
    ...
    def isGoalState(self, state):
        ...
        # print(f"Checking Goal State for {state}: {isGoal}")
        return isGoal
        util.raiseNotDefined()
    ...
    def getSuccessors(self, state):
        ...
        # print(f"Generating successors for state: {state}")
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if nextPosition in self.corners and nextPosition not in newVisitedCorners:
                    ...
                    # print(f"Successor: {nextPosition}, Action: {action}, New Visited: {newVisitedCorners}")
                successors.append(((nextPosition, tuple(newVisitedCorners)), action, 1))

        self._expanded += 1 # DO NOT CHANGE
        # print(f"Successors for {state}: {successors}")
        return successors
...
def cornersHeuristic(state, problem):
    ...
    while unvisitedCorners:
        distances = [util.manhattanDistance(current, corner) for corner in unvisitedCorners]
        minDistance = min(distances)
        heuristicValue += minDistance
        minIndex = distances.index(minDistance)
        current = unvisitedCorners[minIndex]
        unvisitedCorners.pop(minIndex)
...
def foodHeuristic(state, problem):
    ...
    # Use the maximum distance as the heuristic value (admissible and consistent)
